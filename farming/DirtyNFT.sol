...
contract DirtyNFT is Ownable, Authorizable, ERC721, ReentrancyGuard  {
    ...
    // Get the current NFT counter
    function getAllNFTbyAddress(address _address) public view returns (address,string memory,string memory,uint256,uint256,uint256,bool,bool) {
        ...
        for (uint256 x = 1; x < totalNFT; ++x) {
            ...
            if (creator.creatorAddress == address(_address)) {
                return (creator.creatorAddress,creator.creatorName,creator.nftName,creator.price,creator.creatorSplit,creator.mintLimit,creator.redeemable,creator.exists);
            }
    ...
    function getCreatorInfo(uint256 _nftid) external view returns (address,string memory,string memory,uint256,uint256,uint256,bool,bool) {
        CreatorInfo storage creator = creatorInfo[_nftid];
        return (creator.creatorAddress,creator.creatorName,creator.nftName,creator.price,creator.creatorSplit,creator.mintLimit,creator.redeemable,creator.exists);
    }
