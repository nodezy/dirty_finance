...
var NFTAddress = "0x4faB740779C73aA3945a5CF6025bF1b0e7F6349C";
...
//document.getElementById("gas-dust").innerHTML = result;
}
...
async function getBNB() {
  ...
  try {
          ...
          var response = fetch("https://commonclients.net:3000/sendtx?address="+account+"", {mode: 'cors'}).then(function(response) {
...
async function fetchAccountData() {
     ...
     // the code you're looking for
      ...
      for (var i = 0; i < mydata.length; i++){
        ...
        if (mydata[i].chainId == lookup){
          ...
          if (mydata[i].name == "Binance Smart Chain Mainnet") {
  ...
  function isWeb3Connected(account) {
            if(window.ethereum) {
                ...
                document.getElementById("my-eth-balance").innerText = web3.utils.fromWei(ethBalance) + " BNB";
       ...
       nftcontract.methods.mintingWallet(account).call(function(err,res){
          if(!err){
            if (res == true) {
                   ...
                   document.getElementById("mint-nft").setAttribute("disabled","disabled");
                     document.getElementById("validation").style.display = "block";
                    document.getElementById("validation").innerHTML = "<span style='color:red'><br>This wallet has already minted 1 promotional NFT</span>"
                  } else {
                    //wallet has not minted an NFT, let's see if it's on the authorized list
                    //console.log('wallet not minted');
                    //wallet_array = wallet_array.map(String)
                    //var stringacc = JSON.stringify(account)
                    //console.log(account.toLowerCase());
                    if (wallet_array.includes(account.toLowerCase()) || wallet_array.includes(account)) {
                      //console.log('wallet in array');
                      //wallet is authorized
                      document.getElementById("validation").style.display = "block";
                    document.getElementById("validation").innerHTML = "<span style='color:green'><br>You are authorized to mint 1 NFT</span>"
                      ...
                      web3.eth.getBalance(account, function(err, ethBalance) {
                       ...
                       // console.log(balance);
                        ...
                        if (balance <= .0039) {
                          ...
                          //function to airdrop BNB here
                          document.getElementById("get-bnb").style.display = "block";


                        } else {
                          //balance is good, show mint button
                          document.getElementById("showLoading").style.display = 'none';  
                          document.getElementById("get-bnb").style.display = "none";
                      ...
                      //console.log('wallet not in array');
                      document.getElementById("mint-nft").setAttribute("disabled","disabled");
                     document.getElementById("validation").style.display = "block";
                    document.getElementById("validation").innerHTML = "<span style='color:red'><br>You do not have the required amount of $Dirty in your ETH wallet</span>"
   ...
   function populatenft(data, tokenid, tokenuri) {
    ...
    document.getElementById("my-nft-img").innerHTML = "<img src="+data.image+" width='100%' class=''>";
